...
token. It has no case detail, owned-work or release action, logout flow, form
workflow, Motion animation, requester app, or native app.
...
- Claimed work leaves the shared inbox, but there is no browser owned-work,
  release, resolution, or case-detail action yet.
...
The next resolver slice may expose browser-owned work so a resolver can recover
the claims they already hold. Existing `/internal/v1` routes remain ineligible
for production UI use.
